...
export default function PlatformEmailSettings({ onSaved }: PlatformEmailSettingsProps) {
  ...
  const [testRecipient, setTestRecipient] = useState('admin@marketforge.scamspike.com');
  ...
  const handleTestEmail = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/superadmin/platform-email/test', {
        ...
        body: JSON.stringify({
          ...
          inlineConfig: {
            ...
            smtpPassword: config.smtpPassword, // will use if new entered, otherwise server uses stored
            smtpSecurity: config.smtpSecurity,
            senderName: config.senderName,
            senderEmail: config.senderEmail,
            replyToEmail: config.replyToEmail,
            enableProductionEmail: config.enableProductionEmail,
      ...
      setTestResult({
        ...
        recommendation: 'Verify backend API connectivity.'
      });
    } finally {
      setTesting(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Live Testing & Diagnostics Panel */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">Diagnostic Mail Handshake &amp; Live Test</h3>
              <p className="text-[11px] text-slate-500">Send an active test email to verify credentials, socket connectivity, and transport deliverability</p>
            </div>
          </div>
          ...
          {config.lastTestedAt && (
            <div className="flex items-center gap-2 text-xs">
              <span className="text-slate-400 text-[11px]">Last Tested:</span>
              <span className={`px-2 py-0.5 rounded-full font-bold uppercase text-[10px] border flex items-center gap-1 ${
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder="admin@marketforge.scamspike.com"
              className="w-full px-3.5 py-2 border border-slate-200 rounded-xl text-xs text-slate-900 font-mono focus:outline-none focus:border-indigo-500"
            />
          </div>

          <div className="flex flex-wrap items-end gap-2 pt-2 sm:pt-0">
            <button
              type="button"
              disabled={testing || loading}
              onClick={handleTestEmail}
              className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer shadow-sm disabled:opacity-50 h-[38px] mt-auto"
            >
              {testing ? <RotateCw className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
              {testing ? 'Testing Handshake...' : 'Test Email Dispatch'}
            </button>
            ...
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer shadow-md hover:shadow-lg disabled:opacity-50 h-[38px] mt-auto"
            >
              <Zap className="w-3.5 h-3.5" />
              Save &amp; Test Dispatch
            </button>
        ...
        {testResult && (
          <div className={`p-4 rounded-xl border transition animate-fade-in ${
            ...
            <div className="flex items-start gap-3">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 font-bold ${
              ...
              <div className="flex-1 space-y-1.5 text-xs">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="font-bold text-sm">
                    {testResult.success ? '✓ Diagnostic Mail Dispatch Verified' : '✗ Mail Handshake Failed'}
                  </div>
                  {testResult.latencyMs && (
                    <span className="text-[11px] font-mono bg-white/80 border border-slate-200/50 px-2 py-0.5 rounded font-semibold">
                ...
                {testResult.recommendation && (
                  <div className="bg-white/90 p-2.5 rounded-lg border border-slate-200/80 text-[11px] text-slate-700 space-y-1 mt-2">
                    <span className="font-bold text-slate-900 block">Actionable Recommendation:</span>
                    <span>{testResult.recommendation}</span>
                  </div>
                )}

                {testResult.details && (
                  <div className="bg-white/90 p-2.5 rounded-lg border border-slate-200/80 font-mono text-[10px] text-slate-600 space-y-0.5 mt-2">
                    <div>Provider: {testResult.details.provider}</div>
                    <div>Recipient: {testResult.details.recipient}</div>
                    <div>Sender: {testResult.details.sender}</div>
